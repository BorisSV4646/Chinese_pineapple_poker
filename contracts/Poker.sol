...
contract Poker is Ownable {
    ...
    function showdown(
        ...
        require(
            ...
            "Incorrect arr length"
        );

        // verify the player hashes
        for (uint i = 0; i < n; i++) {
            bytes32 givenHash1 = keccak256(
                abi.encodePacked(_keys[i], _cards[i].card1)
            );
            bytes32 givenHash2 = keccak256(
                abi.encodePacked(_keys[i], _cards[i].card2)
            );

            PlayerCardHashes memory hashes = playerHashes[round.players[i]][
                _tableId
            ][table.totalHands];
    ...
    function _allElementsEqual(
        uint[] memory arr
    ) internal pure returns (bool val) {
        uint x = arr[0];
        val = true;
